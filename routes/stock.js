...
router.route("/restock").get(async (req, res) => {
  sql.connect(db, function (err) {
    ...
    con.query(
      `SELECT tblStockOrder.*, [tblCompanies].[CompanyName] + ' - ' + ISNULL([tblCompanies].[OfficeName], '') AS Supplier  FROM tblStockOrder LEFT JOIN tblCompanies ON tblStockOrder.SupplierID = tblCompanies.CompanyID WHERE tblStockOrder.StockOrderID IS NOT NULL AND tblStockOrder.OrderTypeID = 2 ORDER BY tblStockOrder.DeliveryDate DESC;`,
      function (err, record) {
        if (err) {
          console.log(err);
          res.status(500).json({ status: false });
...
router.route("/stockcheck").get(async (req, res) => {
  sql.connect(db, function (err) {
    ...
    con.query(
      "SELECT * FROM tblStockOrder WHERE StockOrderID IS NOT NULL AND OrderTypeID = 3 ORDER BY DeliveryDate DESC;",
      function (err, record) {
        if (err) {
          console.log(err);
          res.status(500).json({ status: false });
...
router.route("/stockOrder1").get(async (req, res) => {
  try {
    ...
    const result = await con
      .request()
      .query(
        `SELECT FORMAT(Quantity * [Price], 'C', 'en-US') AS [Value], tblStockOrderDetails.*, qryStockLevels.StockLevel FROM tblStockOrderDetails LEFT JOIN qryStockLevels ON tblStockOrderDetails.StockItemID = qryStockLevels.StockItemID WHERE ISNULL([Cancelled], 0) = 0;`
      );
    res.status(200).json({
      data: result.recordsets[0],
    });
...
router.route("/addNewContractStockOrder").post(async (req, res) => {
  try {
    ...
    const result = await con.request()
      .query(`SELECT TOP 100 tblStockOrder.StockOrderID, tblStockOrder.DeliveryDate, tblStockOrder.RequestedByID, tblStockOrder.JobID, tblStockOrder.SupplierID, tblStockOrder.DivisionID, tblStockOrder.Notes, tblStockOrder.OrderTypeID, tblStockOrder.RaisedDate, tblStockOrderDetails.StockOrderDetID, tblStockOrderDetails.StockOrderID, tblStockOrderDetails.StockItemID, tblStockOrderDetails.PreEnterQuantity, tblStockOrderDetails.EnteredQuantity, tblStockOrderDetails.Quantity, tblStockOrderDetails.Unit, tblStockOrderDetails.Price, tblStockOrderDetails.AllocatedToID, tblStockOrderDetails.Notes, tblStockOrder.RaisedBy, FORMAT(ISNULL(Quantity * [Price], 0), 'C') AS [Value], tblAllocatedTo.FirstName+' '+tblAllocatedTo.Surname AS AllocatedTo, tblEmployees.FirstName+' '+tblEmployees.Surname AS RequestedBy, 
